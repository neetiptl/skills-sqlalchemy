...
class Model(db.Model):
    ...
    brand = db.relationship("Brand", backref=("brandname"))


    def __repr__(self):
        """Provide helpful representation when printed."""

        return "<Model id=%s year=%s brand_name=%s name=%s >\n" % (self.id, self.year, self.brand_name.encode('utf-8'), self.name)
...
class Brand(db.Model):
    ...
    founded = db.Column(db.SMALLINT, nullable=True)
    headquarters = db.Column(db.String(50), nullable=True)
    discontinued = db.Column(db.SMALLINT, nullable=True)
